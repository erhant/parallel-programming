...
int main(int argc, char *argv[]) {
  ...
  for (int iter = 0; iter < n_iter; iter++) {
    ...
    for (int i = 1; i < height - 1; i++) {
      for (int j = 1; j < width - 1; j++) {
        ...
        north_deriv[k] =
            image[(i - 1) * width + j] - image[k];  // north derivative --- 1 floating point arithmetic operations
        south_deriv[k] =
            image[(i + 1) * width + j] - image[k];  // south derivative --- 1 floating point arithmetic operations
        west_deriv[k] =
            image[i * width + (j - 1)] - image[k];  // west derivative --- 1 floating point arithmetic operations
        east_deriv[k] =
            image[i * width + (j + 1)] - image[k];  // east derivative --- 1 floating point arithmetic operations
        gradient_square = (north_deriv[k] * north_deriv[k] + south_deriv[k] * south_deriv[k] +
                          ...
                          (image[k] * image[k]);  // 9 floating point arithmetic operations
        laplacian = (north_deriv[k] + south_deriv[k] + west_deriv[k] + east_deriv[k]) /
                    image[k];  // 4 floating point arithmetic operations
        num = (0.5 * gradient_square) -
              ((1.0 / 16.0) * (laplacian * laplacian));          // 5 floating point arithmetic operations
    ...
    // THIRD: COMPUTE 2
    // divergence and image update --- 10 floating point arithmetic operations per element -> 10*(height-1)*(width-1) in
    // total
    for (int i = 1; i < height - 1; i++) {
      for (int j = 1; j < width - 1; j++) {
        ...
        divergence = diff_coef_north * north_deriv[k] + diff_coef_south * south_deriv[k] +
                     diff_coef_west * west_deriv[k] +
                     diff_coef_east * east_deriv[k];       // --- 7 floating point arithmetic operations
  ...
  // (HOST) Part VII: Deallocate variables
