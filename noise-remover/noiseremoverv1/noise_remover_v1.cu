...
__global__ void compute1_kernel(unsigned char* image, int width, int height, int n_pixels, float* std_dev, float* north_deriv, float* south_deriv, float* west_deriv, float* east_deriv, float* diff_coef) {
	...
	if (j > 0 && j < width - 1 && i > 0 && i < height - 1) {
		...
		north_deriv[k] = image[k - width] - image[k];	// north derivative --- 1 floating point arithmetic operations
		south_deriv[k] = image[k + width] - image[k];	// south derivative --- 1 floating point arithmetic operations
		west_deriv[k] = image[k - 1] - image[k];	// west derivative --- 1 floating point arithmetic operations
		east_deriv[k] = image[k + 1] - image[k];	// east derivative --- 1 floating point arithmetic operations
		...
		den = (std_dev2 - std_dev[0]) / (std_dev[0] * (1 + std_dev[0])); // 4 floating point arithmetic operations
...
__global__ void compute2_kernel(float* diff_coef, int width, int height, float lambda, float* north_deriv, float* south_deriv, float* west_deriv, float* east_deriv, unsigned char* image) {
	...
	if (j > 0 && j < width - 1 && i > 0 && i < height - 1) {
		...
		diff_coef_north = diff_coef[k];	// north diffusion coefficient (k stays same)
		diff_coef_south = diff_coef[k + width];	// south diffusion coefficient
		diff_coef_west = diff_coef[k];	// west diffusion coefficient (k stays same)
