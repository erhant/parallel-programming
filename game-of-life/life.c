...
int main(int argc, char **argv) {
  ...
  for (ac = 1; ac < argc; ac++) {
    if (MATCH("-n")) {
      ...
      printf(
          "Usage: %s [-n < meshpoints>] [-i <iterations>] [-s seed] [-p prob] [-t numthreads] [-step] [-g <game #>] "
          "[-d]\n",
          argv[0]);
      return (-1);
    }
  ...
  if (!disable_log) {
    char *filename;
    filename =
        (char *)malloc(30 * sizeof(char));  // 30 is arbitrary, but it should be enough to hold "log_SEEDNUMBER.txt"
    if (numthreads == 1) {
  ...
  if (numthreads == 1) {
    ...
    for (t = 0; t < maxiter && population[w_plot]; t++) {
      ...
      if (!disable_display) {
        ...
        // Write the values to the file, we actually dont care about them in terms of visualization, we just want them
        // to be the same! Therefore this is ran by a single thread.
        for (i = 1; i < nx - 1; i++) {
          // Current world only
          for (j = 1; j < ny - 1; j++) {
...
#pragma omp task private(i, j)
          {
            ...
            if (!disable_display) {
              ...
              // Write the values to the file, we actually dont care about them in terms of visualization, we just want
              // them to be the same! Therefore this is ran by a single thread.
              for (i = 1; i < nx - 1; i++) {
                // Current world only
                for (j = 1; j < ny - 1; j++) {
                  fprintf(fp, "%d", currWorld[i][j] ? 1 : 0);
...
#pragma omp single nowait
    ...
    if (!disable_display) {
      ...
      // Write the values to the file, we actually dont care about them in terms of visualization, we just want them to
      // be the same! Therefore this is ran by a single thread.
      for (i = 1; i < nx - 1; i++) {
        // Current world only
        for (j = 1; j < ny - 1; j++) {
          fprintf(fp, "%d", currWorld[i][j] ? 1 : 0);
