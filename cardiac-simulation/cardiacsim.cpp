...
double simulate_CPU(double **E, double **E_prev, double **R, const double alpha,
                    const int n, const int m, const double kk, const double dt,
                    const double a, const double epsilon, const double M1,
                    const double M2, const double b, double *d_E,
                    double *d_E_prev, double *d_R) {
  ...
  for (j = 1; j <= m; j++) {
    for (i = 1; i <= n; i++)
      E[j][i] = E[j][i] - dt * (kk * E[j][i] * (E[j][i] - a) * (E[j][i] - 1) +
                                E[j][i] * R[j][i]);
  }
...
int main(int argc, char **argv) {
  ...
  const double a = 0.1, b = 0.1, kk = 8.0, M1 = 0.07, M2 = 0.3, epsilon = 0.01,
               d = 5e-5;
  ...
  d_E = cudalloc2D(m + 2, n + 2);
  d_E_prev = cudalloc2D(m + 2, n + 2);
  d_R = cudalloc2D(m + 2, n + 2);
  copyToDevice(m, n, d_E, d_E_prev, d_R, E, E_prev, R);
  ...
  // prepare block sizes
  const int xBlocks = ceil((double)m / (double)bx);
  const int yBlocks = ceil((double)n / (double)by);
  const dim3 blockSize(bx, by);
  const dim3 numBlocks(xBlocks, yBlocks);
  ...
  while (t < T) {
    ...
    // run kernel
    kernel_v3<<<numBlocks, blockSize>>>(d_E, d_E_prev, d_R, alpha, epsilon, dt,
                                        kk, a, b, M1, M2, m, n);

    // instead of synchronizing, give swapped params
    kernel_v3<<<numBlocks, blockSize>>>(d_E_prev, d_E, d_R, alpha, epsilon, dt,
                                        kk, a, b, M1, M2, m, n);

    // simulate_CPU(E, E_prev, R, alpha, n, m, kk, dt, a, epsilon, M1, M2, b,
                   d_E, d_E_prev, d_R);
  }

  double time_elapsed = getTime() - t0 - dataTransferTime;

  copyFromDevice(m, n, d_E, d_E_prev, d_R, E, E_prev, R);

  double Gflops = (double)(niter * (1E-9 * n * n) * 28.0) / time_elapsed;
  double BW =
      (double)(niter * 1E-9 * (n * n * sizeof(double) * 4.0)) / time_elapsed;

  cout << "Number of Iterations        : " << niter << endl;
  cout << "Elapsed Time (sec)          : " << time_elapsed << endl;
  cout << "Data Transfer Time (sec)    : " << dataTransferTime << endl;
...
void cmdLine(int argc, char *argv[], double &T, int &n, int &bx, int &by,
             int &plot_freq, int &kernel) {
  ...
  for (ac = 1; ac < argc; ac++) {
    int c;
    while ((c = getopt_long(argc, argv, "n:x:y:t:p:v:", long_options, NULL)) !=
           -1) {
      switch (c) {
      ...
      default:
        printf("Usage:  [-n <domain size>] [-t <final time >]\n\t [-p <plot "
               "frequency>]\n\t[-x <x block geometry> [-y <y block "
               "geometry][-v <Kernel Version>]\n");
        exit(-1);
      }
    }
  }
