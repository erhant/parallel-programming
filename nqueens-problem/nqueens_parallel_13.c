...
{ 
    ...
    for(ac = 1; ac < argc; ac++)
    {
        if(MATCH("-t")) {numthreads = atoi(argv[++ac]);}
        else {
    ...
    #pragma omp single
    {
        ...
        #pragma omp taskgroup // we need taskgroup because we require child-level sychronization, as opposed to scope-limited sync of taskwait.
